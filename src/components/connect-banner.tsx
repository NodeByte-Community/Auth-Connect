...
export function ConnectBanner({ user, onAvatarClick }: Props) {
  return (
    <div className="relative w-full h-48 sm:h-56 md:h-64 rounded-2xl overflow-hidden shadow-lg">
      {/* Gradient background */}
      ...
      {/* Right side: avatar + username + community id */}
      <div className="absolute right-4 sm:right-8 top-1/2 -translate-y-1/2 flex items-center gap-3">
        ...
        <button
          ...
          className="group relative rounded-full ring-4 ring-white/60 hover:ring-teal-300 transition-all shadow-xl hover:scale-105"
        ...
        {/* Mobile: username below avatar */}
        <div className="sm:hidden text-right">
          <div className="text-xs font-semibold text-slate-800 max-w-[80px] truncate">{user.username}</div>
